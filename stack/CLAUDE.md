...
| `modelterms7` | 221 tests, `R CMD check --as-cran` clean apart from the environment notes and the local-only `Remotes` warning, all five phases of `piano_modelterms7.txt` done 2026-08-08 plus the reserved terms `gas()`, `s()` and `te()`. Version `0.6.0`, `NEWS.md` from the first commit. CI, coverage, pkgdown and Pages live. Still reserved: `nl()`, whose block is the Jacobian refreshed per iteration |
...
- `modelterms7` still owes `nl()` (the nonlinear parametric term, whose design block is the Jacobian refreshed at each iteration -- Giovanni's observation, so it stays an additive_term with a refresh hook rather than a third branch) and, for anisotropic tensor smoothing, a sum-of-quadratics penalty in penalties7.
