...
- ~~`modelterms7` owes `nl()`~~ **done 2026-08-08**. modelterms7 is feature complete against `piano_modelterms7.txt`; what it owes now is whatever the model layer asks of it. One contract question is already visible: a structural term's parameters are not penalizable, so `gas(by = , penalty = )` -- a penalty on the group-varying dynamics -- waits on the model layer deciding how a term's own parameters reach a penalty.
