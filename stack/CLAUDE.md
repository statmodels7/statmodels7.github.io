...
| `modelterms7` | 294 tests, `R CMD check --as-cran` clean apart from the environment notes and the local-only `Remotes` warning, all five phases of `piano_modelterms7.txt` done 2026-08-08 and EVERY reserved term implemented -- `gas()`, `s()`, `te()`, `nl()` -- plus `regime()`; first compiled code (src/gas_filter.cpp). Version `0.9.0`, `NEWS.md` from the first commit. CI, coverage, pkgdown and Pages live. Still reserved: `nl()`, whose block is the Jacobian refreshed per iteration |
