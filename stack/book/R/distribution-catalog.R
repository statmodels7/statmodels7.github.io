...
DISTRIBS$laplace2 <- list(
  ...
  curves = list(lambda = c(0.3, 0.7, 1.6)),
  support = "y \\in \\mathbb{R}",
...
DISTRIBS$enet <- list(
  ...
  curves = list(alpha = c(0.02, 0.6, 0.98)),
  support = "y \\in \\mathbb{R}",
...
DISTRIBS$lognormal <- list(
  ...
  curves = list(sigma2 = c(0.2, 1.3, 3)),
  support = "y \\in (0, \\infty)",
...
DISTRIBS$negbin <- list(
  ...
  curves = list(theta = c(0.5, 1.7, 20)),
  support = "y \\in \\{0, 1, 2, \\dots\\}",
...
DISTRIBS$gamma1 <- list(
  ...
  curves = list(phi = c(0.15, 0.4, 1.5)),
  support = "y \\in (0, \\infty)",
...
DISTRIBS$negbin1 <- list(
  ...
  curves = list(theta = c(0.5, 1.5, 10)),
...
DISTRIBS$skewnormal2 <- list(
  ...
  curves = list(gamma1 = c(-0.8, 0, 0.8)),
  support = "y \\in \\mathbb{R}",
...
DISTRIBS$vonmises2 <- list(
  ...
  curves = list(rho = c(0.15, 0.6, 0.95)),
  support = "y \\in [-\\pi, \\pi)",
...
render_distrib_entry <- function(id, rec) {
  ...
  # The picture goes between the algebra and the remark: it says what the
  # parameters do, which is the one thing the formula above does not.
  # A blank line either side keeps knitr from running the image and the
  # sentence after it into a single paragraph.
  cat("\n")
  plot_distrib_entry(rec)
  cat("\n\n", distrib_curve_caption(rec), "\n\n", sep = "")
  cat(gsub("\n\\s+", "\n", rec$note), "\n\n")
}
...
assert_distributions_ok <- function() {
  ...
  # and every record draws a picture that says something
  assert_curves_ok()
